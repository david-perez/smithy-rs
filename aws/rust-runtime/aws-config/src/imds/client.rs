...
impl Builder {
    ...
    pub async fn build(self) -> Result<Client, BuildError> {
        ...
        let connector = expect_connector(config.connector(&HttpSettings {
            timeout_config: timeout::Config::default().with_connect_timeout(Duration::from_secs(1)),
        }));
...
pub(crate) mod test {
    ...
    async fn one_second_connect_timeout() {
        ...
        match resp {
            SdkError::ConstructionFailure(err) if format!("{}", err).contains("TimedOutError") => {} // ok,
            other => panic!(

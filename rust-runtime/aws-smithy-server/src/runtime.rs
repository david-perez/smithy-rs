...
pub(crate) fn json_content_type<B>(req: &RequestParts<B>) -> Result<bool, http::StatusCode> {
    // TODO Replace this with a meaningful error.
    let rejection = http::StatusCode::from_u16(400).unwrap();

    let content_type = if let Some(content_type) = req.headers().ok_or(rejection)?.get(http::header::CONTENT_TYPE) {
        content_type
    } else {
        return Ok(false);
    };
